...
var _workerPair = function (argv, options, name) {
    ...
    sub.on('message', function (msg) {
      ...
      switch (msg.type) {
        ...
        case MESSAGE.STATUS:
          ...
          _pstatus[pid]._time = (new Date()).getTime();
          WRITE_STATUS_FILE(name, pid, msg.data);
          _check_stat_change();
        ...
        case MESSAGE.BOARDCAST:
          if (msg.data && msg.data.data) {
            for (var idx in __WORKERS_LIST) {
              __WORKERS_LIST[idx].each(msg.data);
            }
          }
          break;
  ...
  var _check_stat_change = function () {
    ...
    if (_count !== _running) {
      __GLOBAL_MASTER.emit('state', name, _count, _running);
    }
  ...
  _me.each = function(data) {
    _pobject.forEach(function(sub) {
      try {
        sub.send({
          'type' : MESSAGE.BOARDCAST,
          'data' : data,
        });
      } catch(e) {
    ...
    WRITE_STATUS_FILE = function (name, pid, message) {
      fs.createWriteStream(options.statusfile, {
        'flags' : 'a+', 'encoding' : 'utf-8', 'mode' : 0644,
          ...
          '%d:\t%s\t%d\t%s\n', process.pid, name, pid, JSON.stringify(message)
          ));
    };
...
_Master.prototype.shutdown = function (callback, signal, idx) {
  for (var i in __WORKERS_LIST) {
    if (!idx || i === idx) {
      __WORKERS_LIST[i].stop(callback, signal);
    }
  }
};
...
_Master.prototype.boardcast    = function(data) {
  for (var i in __WORKERS_LIST) {
    if (!idx || i === idx) {
      __WORKERS_LIST[i].each(data);
    }
  }
};
